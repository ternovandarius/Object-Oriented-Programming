...
std::string Movie::to_file() {
    std::string t;
    t=this->title+";"+this->genre+";"+std::to_string(this->year)+";"+std::to_string(this->likes)+";"+this->trailer;
    return t;
}
...
MovieException::MovieException(std::string msg) {
    this->msg=msg;
}
...
void MovieValidator::validate(Movie &m) {
    ...
    if(m.get_year()<=1900 or m.get_year()>2019)
        errors+="Invalid name! ";
    if(m.get_trailer()=="")
        errors+="Invalid trailer URL!\n";
    if(errors!="")
